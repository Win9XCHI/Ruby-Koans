...
class AboutMessagePassing < Neo::Koan
  ...
  def test_sending_a_message_with_arguments
    ...
    assert_equal Array.new([]), mc.add_a_payload
    assert_equal Array.new([]), mc.send(:add_a_payload)

    assert_equal Array.new([3, 4, nil, 6]), mc.add_a_payload(3, 4, nil, 6)
    assert_equal Array.new([3, 4, nil, 6]), mc.send(:add_a_payload, 3, 4, nil, 6)
  end
