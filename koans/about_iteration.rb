...
class AboutIteration < Neo::Koan
  ...
  def test_collect_transforms_elements_of_an_array
    ...
    assert_equal Array.new([11, 12, 13]), new_array

    # NOTE: 'map' is another name for the 'collect' operation
    another_array = array.map { |item| item + 10 }
    assert_equal Array.new([11, 12, 13]), another_array
  end
  ...
  def test_select_selects_certain_items_from_an_array
    ...
    assert_equal Array.new([2, 4, 6]), even_numbers

    # NOTE: 'find_all' is another name for the 'select' operation
    more_even_numbers = array.find_all { |item| (item % 2) == 0 }
    assert_equal Array.new([2, 4, 6]), more_even_numbers
  end
  ...
  def test_all_iteration_methods_work_on_any_collection_not_just_arrays
    ...
    assert_equal Array.new([11, 12, 13]), result

    # Files act like a collection of lines
    File.open("example_file.txt") do |file|
      upcase_lines = file.map { |line| line.strip.upcase }
      assert_equal Array.new(["THIS", "IS", "A", "TEST"]), upcase_lines
    end
